...
// Get item creator and creation info - display AFTER comments
...
if ($item_stmt) {
    ...
    if ($item_result->num_rows > 0) {
        ...
        $avatarHue = $hash;
        // Only show creation info if there are no comments, or add separator if there are
        if ($hasComments) {
            echo '<div style="border-top: 1px solid rgba(102, 126, 234, 0.2); margin: 1rem 0; padding-top: 1rem;"></div>';
        }
        ?>
    ...
    // No comments and no creation info - show message
    if (!$hasComments) {
        echo '<p style="text-align: center; color: #888; padding: 1rem;">No activity yet</p>';
    }
}
$conn->close();
if ($users_conn) {
