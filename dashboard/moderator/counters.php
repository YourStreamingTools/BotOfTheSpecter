...
$hugCounts = $hugCounts ?? [];
$kissCounts = $kissCounts ?? [];
$highfiveCounts = $highfiveCounts ?? [];
...
// Set default data type - check cookie first if consent given, otherwise default to lurkers
...
// If no lurkers data exists, initialize with empty array to prevent errors
if (empty($lurkers)) {
  $lurkers = [];
}

// Prepare the Twitch API request for user data only if we have lurkers
$usernames = [];
if (!empty($lurkers)) {
  ...
  if (isset($userData['data']) && is_array($userData['data'])) {
    ...
    // Set empty usernames array if no lurkers
    foreach ($lurkers as $key => $lurker) {
      $lurkers[$key]['username'] = 'Unknown';
    }
  }
} else {
  $usernames = [];
