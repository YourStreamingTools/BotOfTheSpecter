...
<div class="sp-card">
    ...
    <div class="sp-card-body">
        ...
        <div class="sp-card" style="margin-bottom:1.5rem;" id="eventsub-card">
            <div class="sp-card-header">
                <div class="sp-card-title">
                    <i class="fas fa-satellite-dish"></i>
                    Twitch Extension Bits Webhook
                </div>
                <div id="eventsub-status-badge"></div>
            </div>
            <div class="sp-card-body">
                <p style="color:var(--text-secondary); margin-bottom:1rem;">
                    Subscribe to listen to bingo events when community members use bits. You can unsubscribe at any time and check the status below.
                </p>
                <div id="eventsub-details" style="margin-bottom:1rem; display:none;">
                    <table class="sp-table" style="margin-bottom:0;">
                        <tbody>
                            <tr><td style="color:var(--text-muted); width:130px;">Subscription ID</td><td id="eventsub-id" style="font-family:monospace; font-size:0.8rem;"></td></tr>
                            <tr><td style="color:var(--text-muted);">Status</td><td id="eventsub-sub-status"></td></tr>
                            <tr><td style="color:var(--text-muted);">Created</td><td id="eventsub-created"></td></tr>
                        </tbody>
                    </table>
                </div>
                <div style="display:flex; gap:0.5rem; flex-wrap:wrap;">
                    <button class="sp-btn sp-btn-primary" id="eventsub-subscribe-btn" style="display:none;" onclick="eventsubSubscribe()">
                        <i class="fas fa-plus-circle"></i> Subscribe
                    </button>
                    <button class="sp-btn sp-btn-danger" id="eventsub-unsubscribe-btn" style="display:none;" onclick="eventsubUnsubscribe()">
                        <i class="fas fa-times-circle"></i> Unsubscribe
                    </button>
                    <button class="sp-btn sp-btn-secondary" id="eventsub-refresh-btn" onclick="loadEventSubStatus()">
                        <i class="fas fa-sync-alt"></i> Refresh
                    </button>
                </div>
                <div id="eventsub-message" style="margin-top:0.75rem;"></div>
            </div>
        </div>
...
let _eventsubId = null;

function loadEventSubStatus() {
    const badge = document.getElementById('eventsub-status-badge');
    const details = document.getElementById('eventsub-details');
    const subscribeBtn = document.getElementById('eventsub-subscribe-btn');
    const unsubscribeBtn = document.getElementById('eventsub-unsubscribe-btn');
    badge.innerHTML = '<span class="sp-badge sp-badge-blue"><i class="fas fa-spinner fa-spin"></i> Checking...</span>';
    fetch('bingo_eventsub.php?action=status')
        .then(r => r.json())
        .then(data => {
            if (!data.success) {
                badge.innerHTML = '<span class="sp-badge sp-badge-red">Error</span>';
                showEventSubMsg(data.error || 'Failed to load status', 'danger');
                return;
            }
            const sub = data.subscription;
            if (sub) {
                _eventsubId = sub.id;
                badge.innerHTML = '<span class="sp-badge ' + (sub.status === 'enabled' ? 'sp-badge-green' : 'sp-badge-amber') + '">' + sub.status + '</span>';
                document.getElementById('eventsub-id').textContent = sub.id;
                document.getElementById('eventsub-sub-status').textContent = sub.status;
                document.getElementById('eventsub-created').textContent = sub.created_at ? new Date(sub.created_at).toLocaleString() : '—';
                details.style.display = '';
                subscribeBtn.style.display = 'none';
                unsubscribeBtn.style.display = '';
            } else {
                _eventsubId = null;
                badge.innerHTML = '<span class="sp-badge sp-badge-grey">Not subscribed</span>';
        ...
        .catch(err => {
            badge.innerHTML = '<span class="sp-badge sp-badge-red">Error</span>';
            showEventSubMsg('Error: ' + err.message, 'danger');
        });
}

function eventsubSubscribe() {
    const btn = document.getElementById('eventsub-subscribe-btn');
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Subscribing...';
    fetch('bingo_eventsub.php?action=subscribe', { method: 'POST' })
        .then(r => r.json())
        .then(data => {
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-plus-circle"></i> Subscribe';
            if (data.success) {
                showEventSubMsg('Subscription created successfully!', 'success');
                loadEventSubStatus();
            } else {
                showEventSubMsg('Error: ' + (data.error || 'Unknown error'), 'danger');
            }
        })
        .catch(err => {
            btn.disabled = false;
...
function eventsubUnsubscribe() {
    if (!_eventsubId) return;
    if (!confirm('Remove this EventSub subscription? The bot will stop receiving extension bits events.')) return;
    const btn = document.getElementById('eventsub-unsubscribe-btn');
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Removing...';
    const body = new FormData();
    body.append('subscription_id', _eventsubId);
    fetch('bingo_eventsub.php?action=unsubscribe', { method: 'POST', body: body })
        .then(r => r.json())
        .then(data => {
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-times-circle"></i> Unsubscribe';
            if (data.success) {
                showEventSubMsg('Subscription removed.', 'success');
                loadEventSubStatus();
            } else {
                showEventSubMsg('Error: ' + (data.error || 'Unknown error'), 'danger');
            }
...
function showEventSubMsg(msg, type) {
    const el = document.getElementById('eventsub-message');
    el.innerHTML = '<div class="sp-alert sp-alert-' + type + '" style="margin:0;">' + msg + '</div>';
