...
from bot_modules.database import get_mysql_connection, access_website_database, get_spam_patterns
...
from bot_modules.custom_commands import handle_custom_command, update_custom_count, get_custom_count, fetch_api_response
from bot_modules.twitch.channel_points import channel_point_rewards, process_channel_point_rewards
...
# Database functions are now imported from bot_modules.database
...
async def process_tipping_message(data, source):
    try:
        ...
        if send_message:
            await channel.send(send_message)            # Save tipping data directly in this function
            sqldb = await get_mysql_connection(CHANNEL_NAME)
            try:
                async with sqldb.cursor(aiomysql.DictCursor) as cursor:
                    await cursor.execute(
            ...
            if event_type:
                ...
                # Hype Train Begin Event
                    ...
                    if result and result.get("sound_mapping"):
                        ...
                        asyncio.create_task(websocket_notice(CHANNEL_NAME, API_TOKEN, websocket_logger, event="SOUND_ALERT", sound=sound_file))
                ...
                # Stream Online/Offline Event
                    ...
                    if event_type == "stream.online":
                        ...
                        asyncio.create_task(websocket_notice(CHANNEL_NAME, API_TOKEN, websocket_logger, event="STREAM_ONLINE"))
                    else:
                        bot_logger.info(f"Stream is now offline.")
                        asyncio.create_task(websocket_notice(CHANNEL_NAME, API_TOKEN, websocket_logger, event="STREAM_OFFLINE"))
...
async def FOURTHWALL(data):
    ...
    try:
        channel = BOTS_TWITCH_BOT.get_channel(CHANNEL_NAME)
        await process_fourthwall_event(data, channel, event_logger)
    except Exception as e:
...
class TwitchBot(commands.Bot):
    ...
    async def event_command_error(self, ctx, error: Exception) -> None:
        ...
        if isinstance(error, commands.CommandOnCooldown):
            ...
            bot_logger.error(f"Command: '{command}', Error: {type(error).__name__}, Details: {error}")    # Function to check all messages and push out a custom command.
            ...
            try:
                ...
                if messageContent.startswith('!'):
                    ...
                    if command in builtin_commands or command in mod_commands or command in builtin_aliases:
                        ...
                        return  # It's a built-in command or alias, do nothing more                    # Check if the command exists in the database and respond
                    ...
                    # Use refactored custom command handler
                    await handle_custom_command(command, messageContent, messageAuthor, channel, tz, chat_logger, CHANNEL_NAME)
                # Handle AI responses
    ...
    async def forceoffline_command(self, ctx):
        ...
        try:
            ...
            await sqldb.ensure_closed()    @commands.cooldown(rate=1, per=15, bucket=commands.Bucket.default)
    ...
    async def points_command(self, ctx):
        ...
        try:
            ...
            await sqldb.ensure_closed()    @commands.cooldown(rate=1, per=15, bucket=commands.Bucket.member)
...
# is_valid_twitch_user function is now imported from bot_modules.twitch.vaild_user
...
# Custom command functions are now imported from bot_modules.custom_commands
...
# process_fourthwall_event function is now imported from bot_modules.fourthwall_events
...
# websocket_notice function is now imported from bot_modules.websocket_notice
...
# channel_point_rewards function is now imported from bot_modules.twitch.channel_points
