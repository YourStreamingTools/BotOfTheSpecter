...
# Python 3.12+ removed the implicit auto-create-if-none-running fallback in asyncio.get_event_loop(). TwitchIO 3.x is written for modern asyncio and likely doesn't need this, but bot.py/beta.py do (TwitchIO 2.10.0 still calls it internally) - applying the same harmless guard here for consistency across all three bot versions per the stable/beta/v6 fix policy.
try:
...
async def specter_websocket():
    ...
    while True:
        try:
            ...
            bot_logger.info(
                f"[SPECTER WEBSOCKET] Attempting to connect (attempt {consecutive_failures + 1})"
            )
...
class TwitchBot(commands.AutoBot):
    ...
    async def event_message(self, message):
        ...
        # In v3 EventSub, source_broadcaster is set when a message was forwarded via shared chat (originated from a different channel). Ignore such messages.
        ...
        async with await mysql_handler.get_connection() as connection, connection.cursor(DictCursor) as cursor:
            ...
            try:
                ...
                if 'http://' in AuthorMessage or 'https://' in AuthorMessage:
                    ...
                    pass
                if _early_content and not _early_content.startswith('!'):
                    create_task(word_replace_maybe_echo(messageAuthor, _early_content, is_echo=False))
            except Exception as e:
    ...
    async def word_replace_off_command(self, ctx: commands.Context):
        connection = None
        connection = await mysql_handler.get_connection()
        try:
            async with connection.cursor(DictCursor) as cursor:
    ...
    async def word_replace_on_command(self, ctx: commands.Context):
        connection = None
        connection = await mysql_handler.get_connection()
        try:
            async with connection.cursor(DictCursor) as cursor:
    ...
    async def taskhelp_command(self, ctx: commands.Context):
        global bot_owner
        connection = None
        connection = await mysql_handler.get_connection()
        try:
    ...
    async def tasktimer_command(self, ctx: commands.Context):
        global bot_owner
        connection = None
        connection = await mysql_handler.get_connection()
        try:
            async with connection.cursor(DictCursor) as cursor:
                ...
                if result:
                    ...
                    if action in ['start', 'stop', 'pause', 'resume']:
                        create_task(websocket_notice(event="SPECTER_TIMER_CONTROL", additional_data={
                        ...
                        row = await cursor.fetchone()
                        if row:
                            create_task(websocket_notice(event="SPECTER_SETTINGS_UPDATE", additional_data={
                                "channel_code": API_TOKEN,
    ...
    async def done_command(self, ctx: commands.Context):
        global bot_owner
        connection = None
        connection = await mysql_handler.get_connection()
        try:
    ...
    async def rename_command(self, ctx: commands.Context):
        global bot_owner
        connection = None
        connection = await mysql_handler.get_connection()
        try:
            async with connection.cursor(DictCursor) as cursor:
                ...
                if result:
                    ...
                    create_task(websocket_notice(event="TASK_UPDATE", additional_data={
                        "channel_code": API_TOKEN,
                        "owner": "user",
                        "task": {"id": task_id, "user_id": user_id, "user_name": user_name,
    ...
    async def taskclear_command(self, ctx: commands.Context):
        global bot_owner
        connection = None
        connection = await mysql_handler.get_connection()
        try:
    ...
    async def mytasks_command(self, ctx: commands.Context):
        global bot_owner
        connection = None
        connection = await mysql_handler.get_connection()
        try:
    ...
    async def later_command(self, ctx: commands.Context):
        global bot_owner
        connection = None
        connection = await mysql_handler.get_connection()
        try:
    ...
    async def soon_command(self, ctx: commands.Context):
        global bot_owner
        connection = None
        connection = await mysql_handler.get_connection()
        try:
    ...
    async def backlog_command(self, ctx: commands.Context):
        global bot_owner
        connection = None
        connection = await mysql_handler.get_connection()
        try:
    ...
    async def projects_command(self, ctx: commands.Context):
        global bot_owner
        connection = None
        connection = await mysql_handler.get_connection()
        try:
...
def _syllablize(word):
    dic = _get_hyphenator()
    if dic is not None:
        # Pyphen.inserted() returns the word with hyphens at every break point (e.g. "remember" -> "re-mem-ber"). Words here are pure [A-Za-z], so splitting on '-' is safe. (Pyphen.iterate() yields (head, tail) tuples, NOT syllable chunks - don't use it.)
...
def emit_task_create(task, owner="user"):
    create_task(websocket_notice(event="TASK_CREATE", additional_data={
...
def emit_task_complete(task_id, user_id, user_name, title, project=None, owner="user"):
    create_task(websocket_notice(event="TASK_COMPLETE", additional_data={
...
def emit_project_update(user_id, user_name, change, name=None, old_name=None, task_id=None):
    ...
    create_task(websocket_notice(event="PROJECT_UPDATE", additional_data=payload))
...
def emit_completion_messages_and_events(user_id, user_name, task_id, task_title, award_points, new_total, pending, project=None, owner="user"):
    ...
    if award_points > 0:
        create_task(websocket_notice(event="TASK_REWARD_CONFIRM", additional_data={
...
async def media_songrequest(ctx, connection, bucket_key, cooldown_bucket):
    global bot_owner
    try:
                ...
                if event == "WALKON" and user:
                    ...
                    if MEDIA_MIGRATED:
                            ...
                            for _stem in _walkon_stems:
                                for ext in ['.mp3', '.mp4']:
                                    ...
                                    try:
                                        ...
                                        websocket_logger.error(f"Error checking walkon media URL {media_url}: {e}")
                            ...
                            for ext in ['.mp3', '.mp4']:
                                ...
                                try:
                                    if await http_public_file_exists(walkon_url, session):
                                        ...
                                        walkon_resolved = True
                                        websocket_logger.info(
                                            f"WALKON triggered for {user}: found CDN file {walkon_url}"
                                        )
                                        break
                                except Exception as e:
                                    websocket_logger.error(f"Error checking walkon URL {walkon_url}: {e}")
                        ...
                        if not walkon_resolved:
                            websocket_logger.info(
                                f"WALKON skipped for {user}: no walk-on file on "
        ...
        async with connection.cursor(DictCursor) as cursor:
            ...
            if new_commands:
                ...
                for command in new_commands:
                    ...
                    # Per-viewer social/task commands get a per-user cooldown bucket.
