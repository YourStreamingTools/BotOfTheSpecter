...
async def process_twitch_eventsub_message(message):
    global last_poll_progress_update
    connection = None
    try:
        connection = await mysql_connection()
        async with connection.cursor(DictCursor) as cursor:
...
async def specter_websocket():
    ...
    while True:
        try:
            ...
            # If sticky state still says connected, hard-reset then connect
            ...
            await _force_specter_ws_reset()
            # Already connected is sticky client state — extra settle before retry
...
async def ERROR(data):
    ...
    # Duplicate-session targets the *old* SID; do not tear down a live registered session
...
async def handel_twitch_poll(event=None, poll_title=None, half_time=None, message=None):
    ...
    if event in ("poll.begin", "poll.start"):
        ...
        @routines.routine(seconds=half_seconds, iterations=1, wait_first=True)
            ...
            async with httpClientSession() as session:
                async with session.post("https://api.twitch.tv/helix/moderation/automod/message", headers=headers, json=body) as response:
                    if response.status == 204:
                        twitch_logger.info(f"Denied AutoMod-held message {msg_id}.")
                    else:
                        error_text = await response.text()
                        twitch_logger.error(f"Failed to deny AutoMod-held message {msg_id}. Status Code: {response.status}, Response: {error_text}")
    except Exception as e:
        twitch_logger.error(f"Error denying AutoMod-held message {msg_id}: {e}")
    finally:
        if connection:
            await connection.ensure_closed()
