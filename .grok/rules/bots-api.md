...
| POST | `/api/ops/run_script` | Allowlisted ops only (`refresh_spotify`, `refresh_streamelements`, `refresh_discord`, `refresh_custom_bot`, `refresh_twitch_app_token`, `refresh_youtube`) |
...
8. **Auto-stop if Specter is no longer a moderator.** A background sweep (`./bot/bots_api/mod_check.py`, ~5 min, not the 15s snapshot tick) calls Helix `GET /helix/moderation/moderators?broadcaster_id={id}&user_id=971436498` using the **streamer’s** `website.twitch_bot_access` token (fallback `users.access_token`). Do **not** use `bot_chat_token` (app token cannot list mods). Stop only on HTTP 200 with Specter absent. 401/403/429/5xx/timeouts skip — never treat a dead token as an unmod. Skip Kick, `-self`, and Specter’s own channel (`botofthespecter` / `971436498`). Custom-bot channels still check **Specter** (the custom account is chat-only; Specter still bans/timeouts). Disable with `BOTS_MOD_CHECK_ENABLED=0`.
