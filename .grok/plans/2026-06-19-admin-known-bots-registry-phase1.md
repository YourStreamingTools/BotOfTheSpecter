# Admin-Editable Known-Bots Registry ΓÇö Phase 1 Plan
...
**Spec:** `.grok/specs/2026-06-19-admin-known-bots-registry.md`
...
- **PHP never reads `.env`.** Config comes from `./config/{service}.php` (dev) ΓåÆ `/var/www/config/` (server). The admin page gets its DB handle from `db_connect.php`.
...
## Work item 1 ΓÇö API: DB-backed known-bots infrastructure

The first job is to replace the hardcoded `KNOWN_BOT_LOGINS` set in `api.py` with the table, the seed, the cache, and the read/ensure helpers, then wire those into startup and the leaderboards endpoint. After this work item the leaderboards still behave exactly as before ΓÇö only the *source* of the exclusion list moves from a constant to the DB. Coverage of additional boards comes in the next work item.

**Rename the set to a seed.** The existing `KNOWN_BOT_LOGINS = { ... }` block becomes `DEFAULT_KNOWN_BOTS_SEED` with byte-for-byte the same logins (botofthespecter, nightbot, streamelements, streamlabs, moobot, fossabot, wizebot, and the rest ΓÇö roughly 41 entries). The seed serves two purposes: it populates an empty table on first deploy, and it's the fallback when the table is empty or unreachable. A comment should point readers at the admin page as the place the live list is actually edited.
...
**Point the leaderboards filter at the DB.** In `get_dashboard_leaderboards`, the line that built the exclusion set from the hardcoded constant now awaits `get_known_bots_list()` instead. Everything else in the endpoint ΓÇö the union with the per-channel `watch_time_excluded_users`, the `bot_ph` placeholder string, and the pre-existing watch-time and chat filters ΓÇö stays as it is. That keeps current behaviour intact while the broader coverage lands separately.
...
## Work item 2 ΓÇö API: extend the bot filter to every person-keyed board
...
- **Streaks board** (`analytic_stream_watch_streak`): add the filter to the existing `ORDER BY highest_streak DESC` query. The column here is `user_name` (not `username`) ΓÇö easy to get wrong, so the filter must target `LOWER(user_name)`.
...
## Work item 3 ΓÇö Dashboard admin page (`known_bots.php`)
...
## Work item 4 ΓÇö Navigation, i18n, and styling
...
- the page title and intro paragraph (explaining that these accounts are hidden from every channel's leaderboards ΓÇö watch time, chat, streaks, interactions);
...
The English copy is the source of truth; German and French are full translations. The French file needs its apostrophes escaped (`\'`) since the strings are single-quoted PHP ΓÇö that file is the one most likely to fail a lint if an apostrophe slips through. All four edited PHP files should pass `php -l`.

**Status-badge styles.** Append a small `.kb-status` pill style to `dashboard.css` using existing theme tokens ΓÇö a green-tinted variant for active and a muted variant for disabled ΓÇö rather than a hardcoded palette. If the expected tokens (`--green`, `--text-muted`, `--radius-pill`, or `color-mix`) aren't present, fall back to the nearest tokens the sibling badge/alert components already use; confirm by grepping `dashboard.css` before editing.
...
1. After an API restart, `website.known_bots` exists and is seeded ΓÇö `SELECT COUNT(*) FROM website.known_bots;` returns roughly the seed-set size (~41).
2. `dashboard/admin/known_bots.php` renders the seeded list for an admin, and `admin_access.php` denies a non-admin.
3. Adding a test login (e.g. `mytestbot`) makes it disappear from every leaderboard board within the cache TTL (Γëñ5 min); disabling it brings it back; deleting it removes it for good.
4. A per-channel `watch_time_excluded_users` entry still takes effect ΓÇö confirming the union with the per-channel list is preserved, not replaced.
5. Audit rows are written for each action ΓÇö `SELECT action, target_value FROM website.admin_audit_log WHERE action LIKE 'known_bot_%' ORDER BY id DESC LIMIT 5;` shows the add/toggle/delete entries.

## Phase 2 (separate plan ΓÇö out of scope here)
