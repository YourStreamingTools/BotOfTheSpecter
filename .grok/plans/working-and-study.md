...
**Affected systems**
- `./bot/beta.py` ΓÇö **primary implementation target.** All new chat commands and websocket emits land here first, per [bot-versions.md](../rules/bot-versions.md).
- `./bot/beta-v6.py` ΓÇö port from beta once features are stable.
- `./bot/bot.py` (stable) ΓÇö **not changed** unless a critical fix is needed.
- `./websocket/server.py` + handlers ΓÇö new `USER_POMO_*` events for Phase 3 (existing TASK_* + SPECTER_TIMER_* handlers reused as-is for Phases 1, 2, 4, 5, 6, 7). See ┬º6 for the detailed gap list.
- `./api/api.py` ΓÇö **no changes needed** for any phase. Overlay and dashboard self-serve via PHP; the bot writes MySQL directly per [data-flow.md](../rules/data-flow.md). See ┬º6.
- `./overlay/working-or-study.php` ΓÇö render layer, theme switch, per-viewer pomo display.
- `./overlay/index.css` ΓÇö theme tokens, new pomo set badge styling.
- `./dashboard/working-or-study.php` ΓÇö config UI for themes, command toggles, approval defaults.
- `./dashboard/usr_database.php` ΓÇö schema migrations.
- Per-user MySQL DB ΓÇö table additions/migrations.
...
- `TASK_LIST_SYNC` (full snapshot ΓÇö streamer_tasks + user_tasks)
...
- Reward popups (queue-capped, fade-out ΓÇö landed in #15).
...
### Phase 1 ΓÇö Simple-mode chat commands (no schema changes)
...
| Command | Behaviour |
|---|---|
| `!task <title>` | Create a new user_task for the chatter with status='active'. If they already have an active task, the new one is queued (or refused depending on the decision in ┬º5). |
...
- Bot replies use `send_chat_message()` and are concise, single-line ΓÇö chat clutter is the main risk.
...
### Phase 2 ΓÇö Backlog + multi-task model
...
Status semantics:
- `status='active'` + `backlog_position IS NULL` ΓåÆ currently on the task list
- `status='pending'` + `backlog_position` = integer ΓåÆ queued in backlog (ordered by position)
- `status='completed'` / `status='rejected'` ΓåÆ terminal

Only one row per user can be `status='active'` at a time (enforced in the bot logic, not as a DB constraint ΓÇö keep flexibility for project scoping in Phase 4).
...
### Phase 3 ΓÇö Personal pomo timers per viewer
...
- `USER_POMO_START` ΓÇö broadcast when a pomo begins (overlay shows the inline timer).
- `USER_POMO_UPDATE` ΓÇö periodic countdown (overlay updates the inline label every 5s or 10s; per-second updates would be wasteful across hundreds of viewers).
- `USER_POMO_PHASE` ΓÇö fired on workΓåÆbreak or breakΓåÆwork transition (overlay can chime / animate).
- `USER_POMO_COMPLETE` ΓÇö pomo finished all cycles (overlay clears the badge).
- `USER_POMO_CANCEL` ΓÇö chatter cancelled.
...
### Phase 4 ΓÇö Per-viewer project scoping
...
**Backlog/active are project-scoped**: When project is set, only tasks under that project are visible to `!backlog`, `!mytasks`, `!now <n>`, etc. The active task for a user is scoped to (user_id, project) ΓÇö switching projects pauses the previous project's active task implicitly.

### Phase 5 ΓÇö Theme system
...
- `dark` (current) ΓÇö the existing dark palette
- `peachy` ΓÇö warm coral/peach gradients
- `ocean` ΓÇö cool blue gradients
- `forest` ΓÇö green/earth tones
- `midnight` ΓÇö deep purple with high contrast
...
### Phase 6 ΓÇö Streamer pomo cycle numbering
...
### Phase 7 ΓÇö Unified task list view (optional)
...
Sort order in unified mode: active first (with strikethrough for completed), backlog tasks omitted (or shown muted ΓÇö decision in ┬º5).

## 4. API server (`./api/api.py`) ΓÇö gap analysis
...
**Phases 1ΓÇô7 require no api.py changes.** Every phase's read/write path is covered by the existing PHP + bot + websocket stack.
...
## 5. WebSocket server (`./websocket/server.py`) ΓÇö gap analysis

**What already exists** (no changes needed in Phases 1, 2, 4, 5, 6, 7):

| Event | Direction | Handler |
|---|---|---|
| `TASK_CREATE` | client ΓåÆ broadcast | `handle_task_create` (line ~322) |
| `TASK_UPDATE` | client ΓåÆ broadcast | `handle_task_update` (line ~359) |
| `TASK_COMPLETE` | client ΓåÆ broadcast + bot reward trigger | `handle_task_complete` (line ~327) |
| `TASK_APPROVE` | client ΓåÆ broadcast + bot reward trigger | `handle_task_approve` (line ~364) |
| `TASK_REJECT` | client ΓåÆ broadcast | `handle_task_reject` (line ~370) |
| `TASK_DELETE` | client ΓåÆ broadcast | `handle_task_delete` (line ~375) |
| `TASK_SETTINGS_UPDATE` | client ΓåÆ broadcast | `handle_task_settings_update` (line ~378) |
| `TASK_REWARD_CONFIRM` | client ΓåÆ broadcast | `handle_task_reward_confirm` (line ~383) |
| `TASK_REWARD_TRIGGER` | server ΓåÆ bot only | `emit_task_reward_trigger` (line ~336) |
| `TASK_LIST_SYNC` | server ΓåÆ client on register | inline at line ~592 (empty payload ΓÇö overlay self-hydrates via PHP) |
| `SPECTER_TIMER_CONTROL` / `_COMMAND` / `_STATE` / `_UPDATE` | client ΓåÆ broadcast | `_route_timer_event` at lines ~264ΓÇô276 |
| `SPECTER_SETTINGS_UPDATE` | client ΓåÆ broadcast | (existing, used for theme + timer settings) |
| `SPECTER_PHASE` / `SESSION_STATS` / `STATS_REQUEST` | client ΓåÆ broadcast | (existing) |

**Existing broadcast scoping**:
- `broadcast_to_task_clients_only()` ΓÇö routes to clients whose `name` contains "task".
- `broadcast_to_timer_clients_only()` ΓÇö routes to clients whose `name` contains "timer".
...
|---|---|---|
| `USER_POMO_START` | bot ΓåÆ server ΓåÆ broadcast | Chatter started a personal pomo. Server creates the `user_pomos` row, then broadcasts so the overlay shows the inline badge next to that user's task. |
| `USER_POMO_UPDATE` | server ΓåÆ broadcast | Periodic countdown payload (every ~10s, not per-second). Fired by the server ticker. |
| `USER_POMO_PHASE` | server ΓåÆ broadcast | WorkΓåÆbreak or breakΓåÆwork transition. Drives a chime/animation on the overlay. |
| `USER_POMO_COMPLETE` | server ΓåÆ broadcast | All cycles finished. Overlay clears the badge. |
| `USER_POMO_CANCEL` | bot ΓåÆ server ΓåÆ broadcast | Chatter ran `!pomo cancel`. Server marks the row cancelled. |

**New broadcast scope** (decision in ┬º7):
- Either reuse `broadcast_to_task_clients_only()` (simplest ΓÇö task-aware overlays already get the events) **or** add `broadcast_to_pomo_clients_only()` for finer-grained subscription.
...
- For each: transition phase (workΓåÆbreak or breakΓåÆwork), update row, emit `USER_POMO_PHASE`. If all cycles complete, emit `USER_POMO_COMPLETE`.
- Every ~10s emits `USER_POMO_UPDATE` for each active pomo so overlays can update countdown labels without polling.
- Multi-tenant: ticker iterates across **all** per-user DBs that have a `user_pomos` table. Use the existing usernameΓåÆDB pattern; cache the active DB list with a 60s refresh.
- Uses the existing `./websocket/database_manager.py` (`get_connection(database_name=...)`, `execute_query()`) ΓÇö no new DB infrastructure needed.
...
## 6. Bot ΓåÆ server ΓåÆ DB write path (clarification)
...
3. Bot calls `websocket_notice(event="TASK_CREATE", ...)` (or `TASK_UPDATE` etc.) ΓÇö already implemented in `beta.py` around line 12754 ΓÇö to push the event to the websocket server.
...
3. Websocket server creates the `user_pomos` row itself (centralised state ΓÇö avoids two writers).
...
This split keeps the bot stateless about ongoing pomos ΓÇö the websocket server is the single source of truth ΓÇö which means viewer pomos survive a bot restart cleanly.

## 7. Implementation order recommendation

Stage shipping like this so each stage delivers user value standalone:

1. **Phase 1** ΓÇö chat commands (no schema, immediate visible value).
2. **Phase 6** ΓÇö cycle numbering (cosmetic, low risk, isolates well).
3. **Phase 5** ΓÇö themes (cosmetic, parallel to phase 6).
4. **Phase 2** ΓÇö backlog model (schema + new commands, larger).
5. **Phase 4** ΓÇö projects (depends on phase 2 being live).
6. **Phase 3** ΓÇö personal pomos (biggest engineering ΓÇö needs the ticker, new events).
7. **Phase 7** ΓÇö unified view (last, easy revert if streamers don't like it).

## 8. Open decisions

1. **`!task` collision behaviour when a user already has an active task** ΓÇö refuse with a chat reply ("you already have an active task: X ΓÇö use `!later` to queue"), or auto-push to backlog?
2. **Reward awarding via chat commands** ΓÇö should `!done` issued by a viewer always award points (assuming reward is enabled), or only when an honour-system flag is set in `task_settings`?
3. **Backlog cap per viewer** ΓÇö uncapped (any number) or capped (e.g. 20) to prevent abuse?
4. **Personal pomo concurrency** ΓÇö one active pomo per viewer (cancel + replace), or queue them?
5. **Project name validation** ΓÇö free text up to 100 chars, or restricted character set (alphanumeric + space + dash)? Free text is more flexible but can produce gnarly display strings.
6. **Theme set** ΓÇö five themes proposed; do we ship that many at launch or start with just `dark` + one alternative and add over time?
7. **Cycle badge default** ΓÇö default `show_cycle_badge` to 0 (off) so existing streamers don't see UI change without opt-in, or default to 1 (visible) for the better default behaviour?
8. **Personal pomo broadcast scope** ΓÇö reuse `broadcast_to_task_clients_only()` (simplest, task-aware overlays automatically get pomo events) or add a new `broadcast_to_pomo_clients_only()` so non-task overlays can subscribe to pomo events independently?
