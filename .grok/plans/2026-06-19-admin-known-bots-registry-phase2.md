# Known-Bots Registry ΓÇö Phase 2 (Bot Consumers)

**Goal:** Make the Twitch bot consume the global `website.known_bots` registry ΓÇö unioned with the existing per-channel exclusion lists ΓÇö so known bots stop accruing watch time, earning points, and being welcomed or auto-shouted-out.

**Spec:** `.grok/specs/2026-06-19-admin-known-bots-registry-phase2.md`
...
The per-channel lists keep working exactly as they do today ΓÇö the global registry is purely additive. There's no schema change, no API change, and no dashboard change in this phase; we're only teaching the bot to read a table that already exists.
...
- **`message_counts` behaviour must not change.** Known bots are still *counted* when they chat. Phase 2 only stops them from being welcomed, shouted out, given points, or credited with watch time. This is the subtle one ΓÇö getting the ordering wrong would silently stop counting bots, which we don't want.
...
Both files get the same shape of helper, placed below the `# Functions for all the commands` marker as a module-level async function. It keeps a tiny module-level cache ΓÇö a "loaded at" timestamp and a frozenset of bot logins ΓÇö with a 300-second TTL. Within the TTL it returns the cached set; past it, it re-reads the table.
...
Rows are stripped and lowercased into a set, the cache is refreshed, and the set is returned. The important reliability detail: on **any** exception the loader logs the failure and falls back to the last good cache (empty set on first failure) rather than raising. That way a transient DB hiccup degrades gracefully to per-channel-only exclusion ΓÇö i.e. exactly today's behaviour ΓÇö instead of breaking the call sites that depend on it.
...
In both cases the surrounding symbols ΓÇö `DictCursor`, `time`, `bot_logger` ΓÇö are ones the file already imports and uses, so before wiring the loader in, confirm those names match the live file (and adjust the connection-close idiom if the file's convention differs).
...
The watch-time, points-award, and `!points`-lookup edits are conceptually identical to `beta.py` ΓÇö same union-the-set approach, same lowercasing ΓÇö differing only in the v6 DB idiom for the loader.
...
2. Using the Phase 1 admin page, add a test login that's actively present in the channel (e.g. a secondary account). Within the cache TTL (Γëñ5 min) the bot should stop welcoming it, `!points <login>` should report 0, and its watch time should stop incrementing ΓÇö while real users are unaffected.
...
Whether known bots should *also* be dropped from `message_counts`. This plan takes the recommended position ΓÇö keep counting, skip only welcome/shoutout/points/watch-time. If we later decide counting should be suppressed too, the `beta.py` welcome change simplifies to moving the known-bots check to the shared early-return ahead of the INSERT. That's a deliberate choice to confirm before building, not a detail to flip silently.

## Deploy

Restart the `beta` and `beta-v6` bot processes. Nothing else moves ΓÇö no DB, API, or dashboard deploy.
