...
All dashboard upload pages (makers.php, media.php, alerts.php) write to `/var/www/media/<username>/` (server) via the same `includes/upload_helpers.php` + `move_uploaded_file()`. That path is an **rclone FUSE mount** of Mega S4 (`--allow-other --vfs-cache-mode writes`), not local disk and not s3fs. PHP runs as **www-data**, so the dir must be writable to www-data through FUSE (`allow_other`). `migrate_media.php` creates per-user folders on that mount.
...
Related: [[project_unified_alerts]] (alerts.php uploads here too), [[project_media_player_song_request_hub]], [[project_megas4_public_serving]].
