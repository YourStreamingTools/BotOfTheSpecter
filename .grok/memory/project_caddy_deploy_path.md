...
**How to apply:** Any task touching `web/Caddyfile` — give only the server-side steps above (copy `/var/www/web/Caddyfile` → `/etc/caddy/Caddyfile`, validate, restart), assuming the user has already uploaded the edited file. No git, no `git pull`. Context this arose: switching durable static asset hosts off the FUSE mount onto `reverse_proxy` to MEGA S4 (s3fs served reads with no disk cache → buffered in RAM → 100% RAM crash). rclone later replaced s3fs for PHP I/O; **public HTTP stayed on the S4 proxy** so OBS does not read through FUSE. See [[project_megas4_public_serving]]. **TTS is the exception:** local `file_server` on `/var/www/tts`, never Mega S4 / rclone.
