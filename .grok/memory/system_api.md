...
- **ExchangeRate**: UTC midnight (and admin Check now via `POST /api/exchangerate/quota-sync`) calls upstream `/quota` once, stores `max(0, requests_remaining - 1)` (the quota check itself costs 1), and writes `refresh_day_of_month` into `reset_day`. Bots still decrement the local row on each `!convert`.
