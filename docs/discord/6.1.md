...
* **Implemented dual-logging system** for better user support and debugging:
  * Operators have a central diagnostic log of all bot events
  * Each community has its own event log so users can request diagnostics without seeing other servers
...
* **Fixed stream online/offline notification system**:
  ...
  * Added automatic voice channel name update to online status (🟢) when Twitch API detects stream is live but WebSocket didn't notify
  * Added automatic voice channel name update to offline status (🔴) when Twitch API detects stream is offline
