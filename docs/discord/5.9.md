...
## Testing & Validation
* Recommended steps for testing live notifications and the new LiveChannelManager are outlined in the code comments and Bot README
* Consider running a migration script in production to lowercase existing `online_streams` and `live_notifications` entries for consistency

### Validating the Helix fallback for streamer owners
- Ensure the bot is connected and the channel owner has a valid `channel_mappings` entry with a configured `stream_alert_channel_id`.
- Temporarily block or drop websocket STREAM_ONLINE events for your channel (or monitor without the event)
- Wait for the periodic validation (configured `check_interval`, default 60s) — the bot will perform Helix batch checks for mappings not in `online_streams` and post the announcement if the channel is live, and only for the owner channel (not tracked members)
- Confirm the announcement is posted to the configured `stream_alert_channel_id` and `online_streams` is populated; verify `live_notifications` is inserted to prevent duplicates
