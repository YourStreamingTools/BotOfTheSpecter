...
* Added safe schema migration using column existence checks via INFORMATION_SCHEMA
* `_migrate_server_management_table()` method in both role cogs ensures required JSON columns exist
* Migrations check column existence before adding to prevent errors on concurrent initialization
* Atomic operations ensure race condition-free execution between cogs

## Bug Fixes & Improvements

* Fixed SQL syntax errors in migration queries by removing multi-line statements
* Proper error handling for audit log access restrictions
...
* Safe column migration strategy compatible with all MySQL versions
...
If you find any issues, please report them at the #get-support channel on Discord or on our website: [https://botofthespecter.com/feedback.php](https://botofthespecter.com/feedback.php)
